...
const TENANT_ID = "tenant_gdfc3snnq7ams7fby1rxw36";
...
async function main() {
  const customers = await prisma.pOSCustomer.findMany({ where: { tenantId: TENANT_ID } });
  if (customers.length === 0) throw new Error("This tenant has no customers yet — run the original seed script first, or add customers manually.");

  console.log(`Found ${customers.length} existing customers. Generating ${ORDER_COUNT} orders...`);
  const orders = [];
  const items = [];
  const payments = [];
  const refPrefix = `X${Date.now().toString(36)}`; // unique-ish prefix, avoids clashing with earlier seeded references
  ...
  for (let i = 0; i < ORDER_COUNT; i++) {
    ...
    if (paid > 0) {
      payments.push({
        ...
        amount: paid,
        ref: `RCPT-${refPrefix}-${i}`,
      });
    }

    if ((i + 1) % 5000 === 0) console.log(`  ...prepared ${i + 1}/${ORDER_COUNT}`);
  }

  console.log("Inserting orders...");
  for (let i = 0; i < orders.length; i += 1000) {
    await prisma.pOSOrder.createMany({ data: orders.slice(i, i + 1000) });
    if ((i + 1000) % 5000 === 0) console.log(`  ...inserted ${Math.min(i + 1000, orders.length)}/${orders.length}`);
  }
  ...
  console.log(`Done. ${orders.length} orders, ${items.length} items, ${payments.length} payments added to tenant ${TENANT_ID}.`);
